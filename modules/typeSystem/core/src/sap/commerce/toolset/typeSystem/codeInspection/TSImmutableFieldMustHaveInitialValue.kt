...
class TSImmutableFieldMustHaveInitialValue : TSInspection() {
    ...
    private fun check(
        ...
        if (persistenceType != PersistenceType.DYNAMIC && !write && (!initial || defaultValue == null)) {
            holder.createProblem(
                it,
                severity,
                it.qualifier.stringValue?.let { i18n("hybris.inspections.ts.ImmutableFieldMustHaveInitialValue.details.key", it) }
                    ?: displayName,
                )
        }
    }
